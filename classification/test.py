...
# tf.app.flags.DEFINE_string('weights', './output/model.ckpt-100',
#                             """finetune with a pretrained model""")
tf.app.flags.DEFINE_string('weights', './output/model.ckpt-300',
                            """finetune with a pretrained model""")
...
def test(dataset, ckptfile):
    ...
    with tf.Graph().as_default():
        ...
        for batch_x, batch_y in dataset.batches(batch_size):
            ...
            _info, pred, loss_value = sess.run(
                    [fc8, prediction,  loss,],
                    feed_dict=feed_dict)

            # for p in _info:
            #     info = -tf.reduce_sum(p * tf.log(p))
            #     print(info)
            #     infors.extend(info.tolist())
            infors.extend(_info.tolist())
        ...
        np.save('info.npy',np.array(infors))
        f = open("result.txt", 'w')
        for i in range(0, len(predictions)):
            str_write = str(predictions[i]) + "\n"
            f.write(str_write)
            str_write = str(labels[i]) + "\n"
            f.write(str_write)
            f.write('\n')

        print("Wrong shapes:")
        wrong = np.nonzero(np.array(labels) - np.array(predictions))
        print(wrong)
